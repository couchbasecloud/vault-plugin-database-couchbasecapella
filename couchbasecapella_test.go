...
var (
	testCouchbaseCapellaRole         = fmt.Sprintf(`{"roles":[{"role":"ro_admin"},{"role":"bucket_admin","bucket_name":"%s"}]}`, bucketName)
	testCouchbaseCapellaGroup        = `{"groups":["g1", "g2"]}`
	testCouchbaseCapellaRoleAndGroup = fmt.Sprintf(`{"roles":[{"role":"ro_admin"},{"role":"bucket_admin","bucket_name":"%s"}],"groups":["g1", "g2"]}`, bucketName)
)

func prepareCouchbaseCapellaTestContainer(t *testing.T) (func(), string, int) {
...
func TestDriver(t *testing.T) {
	cleanup, address, port := prepareCouchbaseCapellaTestContainer(t)
	defer cleanup()
	...
	t.Run("Version", func(t *testing.T) { testGetCouchbaseCapellaVersion(t, address) })
	...
	if !pre6dot5 {
		...
		t.Log("Skipping group creation as the Couchbase Capella DB does not support groups")
	}

	t.Run("Init", func(t *testing.T) { testCouchbaseCapellaDBInitialize_TLS(t, address, port) })
	t.Run("Init", func(t *testing.T) { testCouchbaseCapellaDBInitialize_NoTLS(t, address, port) })
	t.Run("Init", func(t *testing.T) { testCouchbaseCapellaDBInitialize_Pre6dot5TLS(t, address, port) })
	t.Run("Init", func(t *testing.T) { testCouchbaseCapellaDBInitialize_Pre6dot5NoTLS(t, address, port) })
	...
	t.Run("Create/Revoke", func(t *testing.T) { testCouchbaseCapellaDBCreateUser(t, address, port) })
	t.Run("Create/Revoke", func(t *testing.T) { testCouchbaseCapellaDBCreateUser_DefaultRole(t, address, port) })
	t.Run("Create/Revoke", func(t *testing.T) { testCouchbaseCapellaDBCreateUser_plusRole(t, address, port) })
	t.Run("Create/Revoke", func(t *testing.T) { testCouchbaseCapellaDBCreateUser_groupOnly(t, address, port) })
	t.Run("Create/Revoke", func(t *testing.T) { testCouchbaseCapellaDBCreateUser_roleAndGroup(t, address, port) })
	t.Run("Rotate", func(t *testing.T) { testCouchbaseCapellaDBRotateRootCredentials(t, address, port) })
	t.Run("Creds", func(t *testing.T) { testCouchbaseCapellaDBSetCredentials(t, address, port) })
...
func testGetCouchbaseCapellaVersion(t *testing.T, address string) {
	var err error
	pre6dot5, err = CheckForOldCouchbaseCapellaVersion(address, adminUsername, adminPassword)
	if err != nil {
		t.Fatalf("Failed to detect Couchbase Capella Version: %s", err)
	}
	t.Logf("Couchbase pre 6.5.0 is %t", pre6dot5)
}

func setupCouchbaseCapellaDBInitialize(t *testing.T, connectionDetails map[string]interface{}) (err error) {
...
func testCouchbaseCapellaDBInitialize_TLS(t *testing.T, address string, port int) {
	t.Log("Testing TLS Init()")

	base64pemRootCA, err := getRootCAfromCouchbaseCapella(fmt.Sprintf("http://%s:%d/pools/default/certificate", address, port))
	if err != nil {
		t.Fatalf("err: %s", err)
	}
	...
	err = setupCouchbaseCapellaDBInitialize(t, connectionDetails)
	if err != nil && pre6dot5 {
		t.Log("Testing TLS Init() failed as expected (no BucketName set)")
	}
}

func testCouchbaseCapellaDBInitialize_NoTLS(t *testing.T, address string, port int) {
...
func testCouchbaseCapellaDBInitialize_Pre6dot5TLS(t *testing.T, address string, port int) {
	...
	setupCouchbaseCapellaDBInitialize(t, connectionDetails)
}

func testCouchbaseCapellaDBInitialize_Pre6dot5NoTLS(t *testing.T, address string, port int) {
...
func testCouchbaseCapellaDBCreateUser(t *testing.T, address string, port int) {
...
func testCouchbaseCapellaDBCreateUser_plusRole(t *testing.T, address string, port int) {
...
func testCouchbaseCapellaDBCreateUser_groupOnly(t *testing.T, address string, port int) {
	if pre6dot5 {
	...
	createReq := dbplugin.NewUserRequest{
		...
		Statements: dbplugin.Statements{
			Commands: []string{fmt.Sprintf(testCouchbaseCapellaGroup)},
		},
		Password:   password,
		Expiration: time.Now().Add(time.Minute),
	}
...
func testCouchbaseCapellaDBCreateUser_roleAndGroup(t *testing.T, address string, port int) {
	if pre6dot5 {
		t.Log("Skipping as groups are not supported pre6.5.0")
		t.SkipNow()
	}
	...
	createReq := dbplugin.NewUserRequest{
		...
		Statements: dbplugin.Statements{
			Commands: []string{testCouchbaseCapellaRoleAndGroup},
		},
		Password:   password,
		Expiration: time.Now().Add(time.Minute),
	}
...
func testCouchbaseCapellaDBRotateRootCredentials(t *testing.T, address string, port int) {
	...
	defer doCouchbaseCapellaDBSetCredentials(t, "rotate-root", "rotate-rootpassword", address, port)
...
func doCouchbaseCapellaDBSetCredentials(t *testing.T, username, password, address string, port int) {
...
func testCouchbaseCapellaDBSetCredentials(t *testing.T, address string, port int) {
	doCouchbaseCapellaDBSetCredentials(t, "vault-edu", "password", address, port)
}
